...
type SrcInputPageProps = {
  navigation: any;
};

const SrcInputPage: React.FC<SrcInputPageProps> = ({ navigation }) => {
  const [inputText, setInputText] = useState<string>('');
  const [searchText, setSearchText] = useState<string>('');

  const {setSourceAddress} = useLocationStore();
  ...
  return (
    <SafeAreaView className="bg-white h-full">
      <View className="flex-row justify-between items-center mt-[24px] mb-[30px] px-[24px]">
        <Text className="text-[22px] font-bold">출발지 주소 확인</Text>
